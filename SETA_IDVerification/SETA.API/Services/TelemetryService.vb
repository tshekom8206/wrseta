...
Namespace Services
    ...
    Public Class TelemetryService
        ...
        ' Configuration
        Private ReadOnly _serviceName As String = "SETA.API"
        ...
        Private _duplicateCount As Long = 0

        ' Per-SETA metrics
        Private ReadOnly _setaVerifications As New Dictionary(Of String, Long)
        Private ReadOnly _setaDuplicates As New Dictionary(Of String, Long)
        Private ReadOnly _setaMetricsLock As New Object()
        ...
        ''' </summary>
        Public Sub RecordVerification(status As String, setaCode As String, durationMs As Double, Optional isDuplicate As Boolean = False)
            Threading.Interlocked.Increment(_verificationCount)
            Threading.Interlocked.Increment(_requestCount)

            ' Track per-SETA metrics
            SyncLock _setaMetricsLock
                If Not String.IsNullOrEmpty(setaCode) Then
                    If Not _setaVerifications.ContainsKey(setaCode) Then
                        _setaVerifications(setaCode) = 0
                    End If
                    _setaVerifications(setaCode) += 1

                    If isDuplicate OrElse status = "RED" Then
                        Threading.Interlocked.Increment(_duplicateCount)
                        If Not _setaDuplicates.ContainsKey(setaCode) Then
                            _setaDuplicates(setaCode) = 0
                        End If
                        _setaDuplicates(setaCode) += 1
                    End If
                End If
            End SyncLock

            Debug.WriteLine($"[OTEL] Verification recorded: Status={status}, SETA={setaCode}, Duration={durationMs}ms")
        End Sub
        ...
        Public Function GetMetrics() As TelemetryMetrics
            Dim setaMetrics As New Dictionary(Of String, SetaMetrics)

            SyncLock _setaMetricsLock
                For Each kvp As KeyValuePair(Of String, Long) In _setaVerifications
                    Dim duplicates As Long = 0
                    If _setaDuplicates.ContainsKey(kvp.Key) Then
                        duplicates = _setaDuplicates(kvp.Key)
                    End If
                    setaMetrics(kvp.Key) = New SetaMetrics() With {
                        .SetaCode = kvp.Key,
                        .Verifications = kvp.Value,
            ...
            Return New TelemetryMetrics() With {
                ...
                .TotalDuplicatesBlocked = _duplicateCount,
                .ActiveSpans = _activeSpans.Count,
                .ServiceName = _serviceName,
                .ServiceVersion = _serviceVersion,
                .Enabled = _enabled,
                .Endpoint = _otlpEndpoint,
                .PerSetaMetrics = setaMetrics
            }
        End Function
                ...
                .resourceSpans = New Object() {
                    New With {
                        ...
                        .scopeSpans = New Object() {
                            New With {
                                .scope = New With {.name = "SETA.API", .version = "1.1.0"},
                                .spans = New Object() {
                                    New With {
    ...
    Public Class TelemetryMetrics
        ...
        Public Property TotalDuplicatesBlocked As Long
        Public Property ActiveSpans As Integer
        Public Property ServiceName As String
        Public Property ServiceVersion As String
        Public Property Enabled As Boolean
        Public Property Endpoint As String
        Public Property PerSetaMetrics As Dictionary(Of String, SetaMetrics)
    End Class

    ''' <summary>
    ''' Per-SETA metrics
    ''' </summary>
    Public Class SetaMetrics
        Public Property SetaCode As String
        Public Property Verifications As Long
        Public Property DuplicatesBlocked As Long
    End Class
