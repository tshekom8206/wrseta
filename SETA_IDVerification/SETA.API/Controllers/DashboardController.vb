...
Namespace SETA.API.Controllers
    ...
    Public Class DashboardController
        ...
        ''' For periods 7 days or less, returns hourly data.
        ''' For periods greater than 7 days, returns daily data.
        ''' </summary>
        Private Function GetVerificationTrends(setaId As Integer, days As Integer) As Object
            ...
            Using conn As New SqlConnection(connectionString)
                ...
                If days <= 7 Then
                    ...
                    sql = "
                        SELECT
                            DATEADD(HOUR, DATEDIFF(HOUR, 0, VerifiedAt), 0) AS VerificationHour,
                        ...
                        GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, VerifiedAt), 0)
                        ORDER BY DATEADD(HOUR, DATEDIFF(HOUR, 0, VerifiedAt), 0)"
                Else
                ...
                Using cmd As New SqlCommand(sql, conn)
                    ...
                    Using reader As SqlDataReader = cmd.ExecuteReader()
                        While reader.Read()
                            If days <= 7 Then
                                ' Include time in the date format for hourly data
                                trendList.Add(New With {
                                    .date = reader.GetDateTime(0).ToString("yyyy-MM-dd HH:mm"),
            ...
            Return New With {
                .days = days,
                .granularity = If(days <= 7, "hourly", "daily"),
                .trends = trendList
            }
        End Function
