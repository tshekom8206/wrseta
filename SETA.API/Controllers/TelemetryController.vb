    ...
    Public Class TelemetryController
        ...
        Public Function GetPrometheusMetrics() As IHttpActionResult
            Try
                ...
                sb.AppendLine("# HELP seta_api_requests_total Total number of API requests")
                sb.AppendLine("# TYPE seta_api_requests_total counter")
                sb.AppendLine($"seta_api_requests_total {metrics.TotalRequests}")
                sb.AppendLine()

                sb.AppendLine("# HELP seta_api_errors_total Total number of API errors")
                sb.AppendLine("# TYPE seta_api_errors_total counter")
                sb.AppendLine($"seta_api_errors_total {metrics.TotalErrors}")
                sb.AppendLine()

                sb.AppendLine("# HELP seta_verifications_total Total number of ID verifications")
                sb.AppendLine("# TYPE seta_verifications_total counter")
                sb.AppendLine($"seta_verifications_total {metrics.TotalVerifications}")
                sb.AppendLine()

                sb.AppendLine("# HELP seta_duplicates_blocked_total Total number of duplicate enrollments blocked")
                sb.AppendLine("# TYPE seta_duplicates_blocked_total counter")
                sb.AppendLine($"seta_duplicates_blocked_total {metrics.TotalDuplicatesBlocked}")
                sb.AppendLine()

                sb.AppendLine("# HELP seta_active_spans Current number of active trace spans")
                sb.AppendLine("# TYPE seta_active_spans gauge")
                sb.AppendLine($"seta_active_spans {metrics.ActiveSpans}")
                sb.AppendLine()

                sb.AppendLine("# HELP seta_api_info API service information")
                sb.AppendLine("# TYPE seta_api_info gauge")
                sb.AppendLine($"seta_api_info{{service=""{metrics.ServiceName}"",version=""{metrics.ServiceVersion}""}} 1")
